...
public class ProxyMessagingManager implements PluginMessageListener, Listener {
    ...
    public void sendPlayerToServerAndTeleport(Player player, String targetServer, Location location) {
        plugin.getDebugLogger().log(
                "Preparing to send " + player.getName() + " to server '" + targetServer +
                        "' for home teleport via " + plugin.getProxyType().name() + " protocol."
        );
        ...
        String proxyChannel = (plugin.getProxyType() == JustTeams.ProxyType.VELOCITY) ? VELOCITY_CHANNEL : BUNGEE_CHANNEL;
        player.sendPluginMessage(plugin, proxyChannel, forwardMessage.toByteArray());
        plugin.getDebugLogger().log(plugin.getProxyType().name() + ": Forward message sent to '" + targetServer + "'.");
