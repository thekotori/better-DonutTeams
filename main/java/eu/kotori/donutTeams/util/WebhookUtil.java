...
public class WebhookUtil {
    ...
    private static String getWebhookUrl() {
        ...
        try {
            ...
            DonutTeams.getInstance().getLogger().warning("[DonutTeams Webhook] Failed to decode Webhook URL. It might be corrupted.");
            return null;
        }
    }
    ...
    public static void sendStartupNotification(DonutTeams plugin) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                embed.addProperty("title", "DonutTeams Statistics");
                ...
                footer.addProperty("text", "DonutTeams by " + author);
                ...
                connection.setRequestProperty("User-Agent", "DonutTeams-Statistics");
                ...
                if (responseCode >= 300) {
                    plugin.getLogger().warning("[DonutTeams Webhook] Request failed with response code: " + responseCode);
                }
                connection.disconnect();

            } catch (Exception e) {
                plugin.getLogger().warning("[DonutTeams Webhook] Failed to send startup notification: " + e.getMessage());
            }
        });
    }
